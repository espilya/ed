...
public:
   ...
   iterator erase(iterator const& it) {
       iterator nextIt = it;
       nextIt.next(); // En el caso peor es lineal respecto al tamano de la tabla. En el mejor es constante
       auto act = *it;  
       int i = hash(act.clave) % array.size(); // constante
       Link ant, pos = array[i];
       if (localizar(act.clave, ant, pos)) { // En el caso peor es lineal. En el mejor es constante
           if (ant == nullptr)
               array[i] = pos->sig;
           else
               ant->sig = pos->sig;
           ...
           throw std::out_of_range("Intento de eliminar un elemento que no existe");
           return nullptr;
       }
   }
